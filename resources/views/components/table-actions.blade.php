<div class="flex items-center space-x-2">
  ...
  <!-- Delete -->
  <form action="{{ $rute_activate }}" method='post'>
    @csrf
    @method('PATCH')
        ...
        type="submit"
        aria-label="Hapus"
        class="{{ $row->flag == 1 ? 'bg-green-600 hover:bg-green-700 focus:ring-green-300' : 'bg-red-600 hover:bg-red-700 focus:ring-red-300' }} text-white focus:ring-4 focus:outline-none  rounded-lg p-2 inline-flex items-center"
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
          <path stroke-linecap="round" stroke-linejoin="round"
